...
must_haves:
  truths:
    - "Running /gsd:new-agent starts a guided conversation: purpose → mode → trigger → output type → mode-specific details → decision gate → write"
    - "The conversation branches correctly: advisory agents get criteria + severity questions; autonomous agents get scope + commit questions — not both"
    - "No files are written until the user selects 'Create agent' at the decision gate in step 8"
    - "The decision gate displays the complete assembled role block (and agent file content for autonomous) before asking for confirmation"
    - "On confirmation, advisory agents get only a TEAM.md role block; autonomous agents also get agents/gsd-agent-{slug}.md"
    - "TEAM.md is updated Read-then-Write: frontmatter roles: list includes new slug, role block appended with --- separator"
    - "If TEAM.md does not exist, it is created with version: 2 frontmatter (not version: 1)"
    - "Commit fires via gsd-tools.cjs with appropriate --files list after writing"
  artifacts:
    - path: "workflows/new-agent.md"
      provides: "9-step guided conversation workflow for agent creation"
      contains: "check_team_md"
    - path: "workflows/new-agent.md"
      provides: "Mode-branched conversation: gather_advisory_details and gather_scope"
      contains: "gather_advisory_details"
    - path: "workflows/new-agent.md"
      provides: "Decision gate with full preview and Create/Edit/Cancel options"
      contains: "decision_gate"
    - path: "workflows/new-agent.md"
      provides: "Write step that commits via gsd-tools.cjs"
      contains: "write_agent"
  key_links:
    - from: "workflows/new-agent.md gather_mode step"
      to: "gather_advisory_details OR gather_scope"
      via: "conditional branch on mode == advisory | autonomous"
      pattern: "gather_advisory_details.*advisory|gather_scope.*autonomous"
    - from: "workflows/new-agent.md decision_gate step"
      to: "write_agent step"
      via: "user selects 'Create agent'"
      pattern: "write_agent"
    - from: "workflows/new-agent.md write_agent step"
      to: "gsd-tools.cjs commit"
      via: "node ~/.claude/get-shit-done/bin/gsd-tools.cjs commit"
      pattern: "gsd-tools.cjs commit"
---

<objective>
Create `workflows/new-agent.md` — the full 9-step guided conversation workflow for `/gsd:new-agent`.

Purpose: This is the core deliverable for Phase 11. The workflow guides the user through creating a complete agent definition, branching on mode (advisory vs autonomous) to ask only relevant questions, showing a full definition preview at the decision gate, and writing on explicit confirmation only.

Output: `workflows/new-agent.md` — a GSD workflow file following the `<purpose>` / `<inputs>` / `<step>` structure, with 9 named steps that implement CREA-01 through CREA-04.
</objective>
...
<task type="auto">
  <name>Task 1: Write workflows/new-agent.md — 9-step guided conversation workflow</name>
  <files>workflows/new-agent.md</files>
  <action>
Create `D:/GSDteams/workflows/new-agent.md` using the Write tool with the following complete content. This file follows the same `<purpose>` / `<inputs>` / `<step>` structure as `workflows/new-reviewer.md`. The step sequence is: check_team_md → gather_purpose → gather_mode → gather_trigger → gather_output_type → gather_advisory_details → gather_scope → decision_gate → write_agent.

Write the file with this exact content (do not truncate or abbreviate any step):
...
CRITICAL CONSTRAINTS to enforce during writing:
- Mode branch is strict: `gather_advisory_details` fires ONLY when mode == "advisory"; `gather_scope` fires ONLY when mode == "autonomous"
- No writes occur before the decision_gate step — steps 1-8 are read-only
- When creating a new TEAM.md, use `version: 2` (not version: 1)
- Advisory agents do NOT get a `scope:` block in their TEAM.md role entry
- Autonomous agents do NOT get severity/routing fields in their TEAM.md role entry
- The decision gate must show "Cancel" as an option that exits with no writes
- Commit uses `~/.claude/get-shit-done/bin/gsd-tools.cjs` (installed path, not source path)
  </action>
  <verify>
Read `D:/GSDteams/workflows/new-agent.md` and verify:
1. File exists and has `&lt;purpose&gt;`, `&lt;inputs&gt;`, and 9 `&lt;step name="..."&gt;` blocks
2. Step names present: check_team_md, gather_purpose, gather_mode, gather_trigger, gather_output_type, gather_advisory_details, gather_scope, decision_gate, write_agent
3. `gather_advisory_details` contains conditional text "Skip this step entirely if mode == autonomous"
4. `gather_scope` contains conditional text "Skip this step entirely if mode == advisory"
5. `decision_gate` shows the role block preview and has Cancel / Edit / Create options
6. `write_agent` contains the gsd-tools.cjs commit call using `~/.claude/get-shit-done/bin/gsd-tools.cjs`
7. `write_agent` contains "version: 2" for the new TEAM.md creation path
8. No Write tool call appears in any step except `write_agent`
  </verify>
  <done>
`workflows/new-agent.md` exists with all 9 steps. The mode branch is correctly implemented: advisory agents reach gather_advisory_details and skip gather_scope; autonomous agents reach gather_scope and skip gather_advisory_details. The decision gate shows a full preview with Cancel/Edit/Create options. The write step creates version: 2 TEAM.md when needed and commits via gsd-tools.cjs.
  </done>
</task>
...
1. Verify `D:/GSDteams/workflows/new-agent.md` exists
2. Count `&lt;step name=` occurrences — must be exactly 9
3. Verify "gather_advisory_details" and "gather_scope" both appear in the file
4. Verify "decision_gate" step has "Cancel" option
5. Verify "write_agent" step has `gsd-tools.cjs commit` and `version: 2`
6. Verify no Write tool calls appear outside the write_agent step description
</verification>

<success_criteria>
`workflows/new-agent.md` is a complete, executable GSD workflow file that:
- Starts with `&lt;purpose&gt;` listing the 9-step sequence
- Has 9 named steps implementing CREA-01 through CREA-04
- Branches correctly on mode: advisory gets criteria/severity questions, autonomous gets scope/commit questions
- Shows a full definition preview at the decision gate before any write
- Writes only in write_agent step, commits via gsd-tools.cjs
- Creates new TEAM.md with version: 2 (not version: 1)
</success_criteria>

<output>
After completion, create `.planning/phases/11-agent-creation-new-agent/11-01-SUMMARY.md`
</output>
